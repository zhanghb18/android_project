...
public class HttpHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, FullHttpRequest msg) {
        ...
        try {
            ...
            /** 获取操作类型 */
            String bitTypeStr = requestParams.getString(KeyConstant.BIZ_TYPE);
            if (bitTypeStr == null)
                throw new CourseWarn(SystemErrorEnum.BIZ_TYPE_NOT_EXIST);
            try {
                bizTypeEnum = BizTypeEnum.valueOf(bitTypeStr);
    ...
    private void writeResponse(ChannelHandlerContext ctx, String content) {
        ...
        if (!hasPreSession) {
            String encodeCookie = ServerCookieEncoder.STRICT.encode(NameConstant.HTTP_SESSION_NAME, httpSession.getSessionId());
